...
These tests run against the local PostgreSQL fixture database with mocked LLM
services (see ``conftest.pg_test_env`` and ``conftest.mock_llm``), making them
fully deterministic.
"""
...
TEST_DB = "blog_small"
TEST_EXTRA_DB = "ecommerce_medium"

pytestmark = [pytest.mark.usefixtures("pg_test_env", "mock_llm")]
...
class TestMultiDatabaseRouting:
    """Integration tests for multi-database configuration and routing.

    Uses EXTRA_DATABASES to register a second fixture database and verifies
    that requests are routed to the pool/executor of the requested database.
    """

    @pytest.fixture(autouse=True)
    def extra_database_env(self, pg_test_env, monkeypatch):
        """Register the second fixture database via EXTRA_DATABASES.

        Depends on ``pg_test_env`` so it runs after it (which deletes
        EXTRA_DATABASES) and the extra database survives.
        """
        extra = [
            {
                "host": "localhost",
        ...
        monkeypatch.setenv("EXTRA_DATABASES", json.dumps(extra))

    @pytest.mark.asyncio
    async def test_both_databases_pools_created(self):
        """Lifespan should create pools for primary and extra databases."""
        async with lifespan(mcp):
            result = await query(
                question="SELECT 1 as which",
                database=TEST_DB,
                return_type="sql",
            )
            ...
            result = await query(
                question="SELECT 1 as which",
                database=TEST_EXTRA_DB,
                return_type="sql",
            )
            assert result.get("success") is True

    @pytest.mark.asyncio
    async def test_execution_routes_to_requested_database(self):
        """Result queries execute on the requested database's pool."""
        async with lifespan(mcp):
            result = await query(
    ...
    @pytest.mark.asyncio
    async def test_auto_select_requires_database_with_multiple(self):
        """Auto-select must fail when several databases are configured."""
        async with lifespan(mcp):
            result = await query(
                question="SELECT 1",
                return_type="sql",
            ...
            available = result["error"]["details"]["available_databases"]
            assert TEST_DB in available
            assert TEST_EXTRA_DB in available

    @pytest.mark.asyncio
    async def test_unknown_database_rejected(self):
        """Unknown database names produce a structured error."""
        async with lifespan(mcp):
            result = await query(
                question="SELECT 1",
                database="no_such_database_xyz",
                return_type="result",
            )
