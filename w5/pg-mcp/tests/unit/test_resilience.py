...
class TestRetryAsync:
    """Test cases for the retry_async exponential backoff helper."""

    @pytest.mark.asyncio
    async def test_success_first_attempt(self) -> None:
        """Successful operation should return immediately without retries."""
        calls: list[int] = []
        ...
        result = await retry_async(operation, operation_name="test")
        assert result == "ok"
        assert len(calls) == 1
    ...
    async def test_non_retryable_error_propagates_immediately(self) -> None:
        """Exceptions not in retryable should propagate without retrying."""
        attempts: list[int] = []
    ...
    async def test_exponential_backoff_timing(self) -> None:
        """Delays between attempts should grow by the backoff factor."""
        ...
        # First retry after ~0.1s, second after ~0.2s (exponential growth)
        first_gap = timestamps[1] - timestamps[0]
        second_gap = timestamps[2] - timestamps[1]
        assert first_gap >= 0.09
        assert second_gap >= 0.18

    @pytest.mark.asyncio
    async def test_retryable_error_types(self) -> None:
        """Only configured exception types should trigger retries."""
        attempts: list[int] = []
