...
LIMIT 10;"""
    ...
    async def test_generate_records_token_usage(
        ...
        """Test that token usage from the API response metadata is recorded."""
        mock_response = MagicMock()
        mock_response.choices = [MagicMock(message=MagicMock(content="```sql\nSELECT 1;\n```"))]
        ...
        assert generator.last_tokens_used == 123

    @pytest.mark.asyncio
    async def test_generate_without_usage_keeps_tokens_none(
        self, generator: SQLGenerator, mock_schema: DatabaseSchema
    ) -> None:
        """Test that missing usage metadata leaves token tracking as None."""
        mock_response = MagicMock()
        mock_response.choices = [MagicMock(message=MagicMock(content="```sql\nSELECT 1;\n```"))]
