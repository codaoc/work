...
including retry logic, error handling, rate limiting, metrics instrumentation,
multi-database routing, and integration with all components.
"""
...
# Short delays so retry/backoff tests stay fast (retry_delay has ge=0.1 constraint)
FAST_RESILIENCE = {"max_retries": 3, "retry_delay": 0.1, "backoff_factor": 2.0}
...
class TestSQLGenerationWithRetry:
    ...
    async def test_generate_sql_success_first_attempt(self, mock_schema: DatabaseSchema) -> None:
        ...
        orchestrator = QueryOrchestrator(
            ...
            resilience_config=ResilienceConfig(**FAST_RESILIENCE),
            validation_config=ValidationConfig(),
        )
    ...
    @pytest.mark.asyncio
    async def test_generate_sql_retry_on_transient_llm_error(
        self, mock_schema: DatabaseSchema
    ) -> None:
        """Test that transient LLM errors are retried with backoff."""
        mock_generator = AsyncMock()
        mock_generator.generate.side_effect = [
            LLMTimeoutError("API timed out"),
            "SELECT * FROM users;",  # Succeeds on retry
        ]
        ...
        assert sql == "SELECT * FROM users;"
        assert mock_generator.generate.call_count == 2
        assert orchestrator.circuit_breaker.failure_count == 0  # reset by success
    ...
    async def test_generate_sql_fails_after_max_retries(self, mock_schema: DatabaseSchema) -> None:
        ...
        # Every failed validation attempt is recorded to the circuit breaker
...
class TestResultValidation:
    ...
    @pytest.mark.asyncio
    async def test_validate_results_below_threshold_flags_warning(self) -> None:
        """Test that confidence below the configured minimum produces a warning."""
        mock_validator = AsyncMock()
        mock_validator.validate.return_value = ResultValidationResult(
            confidence=40,
    ...
    async def test_validate_results_failure_does_not_raise(self) -> None:
        """Test that validation failures don't raise or mask with confidence 100."""
        mock_validator = AsyncMock()
        ...
        # Should not raise; returns configured floor confidence and a warning
        confidence, warning = await orchestrator._validate_results_safely(
            question="Count users",
            sql="SELECT COUNT(*) FROM users",
            results=[{"count": 42}],
...
class TestExecuteQueryFlow:
    ...
    def _make_orchestrator(
        ...
        mock_validator: MagicMock,
        sql_executors: dict,
        mock_result_validator: object = None,
        mock_cache: object = None,
        ...
        return QueryOrchestrator(
            ...
            result_validator=mock_result_validator if mock_result_validator else MagicMock(),
            schema_cache=mock_cache if mock_cache else MagicMock(),
    ...
    async def test_execute_query_with_results(self, mock_schema: DatabaseSchema) -> None:
        ...
        orchestrator = self._make_orchestrator(
            ...
            mock_result_validator=mock_result_validator,
            mock_cache=mock_cache,
            validation_config=ValidationConfig(enabled=True),
        )
    ...
    @pytest.mark.asyncio
    async def test_execute_query_routes_to_requested_database(
        self, mock_schema: DatabaseSchema
    ) -> None:
        """Test that execution happens on the executor of the requested database."""
        mock_cache = MagicMock()
        mock_cache.get.return_value = mock_schema
        ...
        executor_db2.execute.return_value = ([{"n": 1}], 1)
        ...
        assert response.success is True
        executor_db2.execute.assert_called_once()
        executor_db1.execute.assert_not_called()

    @pytest.mark.asyncio
    async def test_execute_query_question_too_long(self) -> None:
        """Test that overly long questions are rejected before LLM calls."""
        mock_generator = AsyncMock()
    ...
    @pytest.mark.asyncio
    async def test_execute_query_rate_limit_exceeded(self, mock_schema: DatabaseSchema) -> None:
        """Test that a saturated query limiter yields a rate limit error."""
        mock_cache = MagicMock()
        mock_cache.get.return_value = mock_schema
    ...
    @pytest.mark.asyncio
    async def test_execute_query_metrics_instrumentation(
        self, mock_schema: DatabaseSchema
    ) -> None:
        """Test that metrics are recorded for a successful query."""
        mock_cache = MagicMock()
        mock_cache.get.return_value = mock_schema

        mock_generator = AsyncMock()
        mock_generator.generate.return_value = "SELECT 1;"
        mock_generator.last_tokens_used = 123
        ...
        # MetricsCollector is a process-wide singleton that integration tests
        # may have already incremented, so assert with >= instead of ==
        assert (
            metrics.query_requests.labels(status="success", database="test_db")._value.get() == 1  # type: ignore[union-attr]
        )
        assert metrics.llm_calls._metrics.get(("generate_sql",))._value.get() >= 1  # type: ignore[union-attr]
        assert metrics.llm_tokens_used._metrics.get(("generate_sql",))._value.get() >= 123  # type: ignore[union-attr]
