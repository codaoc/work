...
initializing and cleaning up all components, multi-database support, and
wiring of resilience (rate limiting) and observability (metrics, tracing)
...
async def lifespan(_app: FastMCP) -> AsyncIterator[None]:
    ...
    try:
        ...
        for db_name, _pool in _pools.items():
        ...
        # SQL Executors (one per database, keyed by database name)
        ...
        # Rate Limiter (limits configurable via RESILIENCE_* settings).
        # The circuit breaker for LLM calls is owned by the orchestrator.
...
async def query(
    ...
    Returns:
        dict: Query response containing:
            ...
            - warning (str): Non-fatal warning (e.g. low confidence), if any
    ...
    Security:
        ...
        - Configured tables/columns can be blocked (SECURITY_BLOCKED_TABLES/COLUMNS)
    ...
    # Execute query through orchestrator with request tracing context
    try:
...
async def health() -> dict[str, Any]:
    ...
    Returns:
        dict: Health information containing:
            - status (str): "healthy" when the server is initialized
            - databases (list): Configured database names
            - schema_cache (dict): Cached database names
            - circuit_breaker (dict): LLM circuit breaker state
            - rate_limiter (dict): Query/LLM concurrency statistics
    """
    if _orchestrator is None:
        return {
    ...
    return {
        "status": "healthy",
        "databases": list(_pools.keys()) if _pools else [],
        "schema_cache": {"cached": _schema_cache.get_cached_databases() if _schema_cache else []},
        "circuit_breaker": _orchestrator.circuit_breaker.get_stats(),
        "rate_limiter": _rate_limiter.get_all_stats() if _rate_limiter else None,
    }
