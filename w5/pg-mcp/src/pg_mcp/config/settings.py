...
class SecurityConfig(BaseSettings):
    ...
    blocked_tables: list[str] = Field(
        default_factory=list, description="Table names that queries must not access"
    )
    blocked_columns: list[str] = Field(
        default_factory=list, description="Column names that queries must not access"
    )
    allow_explain: bool = Field(
        default=False, description="Allow EXPLAIN statements for query plan inspection"
    )
...
class ResilienceConfig(BaseSettings):
    ...
    query_rate_limit: int = Field(
        default=10, ge=1, le=1000, description="Max concurrent database queries"
    )
    llm_rate_limit: int = Field(
        default=5, ge=1, le=1000, description="Max concurrent LLM API calls"
    )
...
class Settings(BaseSettings):
    ...
    # Additional databases (env: EXTRA_DATABASES as a JSON array of objects).
    # The primary database stays configured via the flat DATABASE_* variables
    # for backwards compatibility.
    extra_databases: list[DatabaseConfig] = Field(default_factory=list)
    ...
    def validate_databases(self) -> "Settings":
        """Ensure at least one database and unique database names."""
        names = [db.name for db in self.databases]
        if not names:
            raise ValueError("At least one database must be configured")
        duplicates = {n for n in names if names.count(n) > 1}
        if duplicates:
            raise ValueError(f"Database names must be unique, got duplicates: {sorted(duplicates)}")
        return self
