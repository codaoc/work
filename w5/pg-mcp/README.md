...
除了通过 `DATABASE_*` 变量配置的主数据库外，还可以通过 `EXTRA_DATABASES`（JSON 数组）注册更多数据库，并在查询时用 `database` 参数指定目标：
...
- 只配置一个数据库时，`database` 参数可省略（自动选择唯一数据库）
- 配置多个数据库时，`database` 参数为必填
- 每个数据库拥有独立的连接池与 SQL 执行器，共享同一套安全校验配置
- 可通过 `health` 工具查看当前配置的数据库、熔断器与限流器状态
...
`warning` 字段在结果为非致命警告时给出提示，例如置信度低于 `VALIDATION_MIN_CONFIDENCE_SCORE` 或结果校验失败（此时置信度会被置为该阈值而不是伪造满分）。
...
| `EXTRA_DATABASES`          | 额外数据库（JSON 数组，见下方多数据库说明） | `[]` |
...
| `OPENAI_MAX_TOKENS`  | 每次请求的最大 token 数（上限 4096） | `2000`         |
...
| `SECURITY_BLOCKED_COLUMNS`        | 逗号分隔的列黑名单        | 空                |
| `SECURITY_ALLOW_EXPLAIN`          | 允许 EXPLAIN 语句         | `false`           |
...
| `RESILIENCE_QUERY_RATE_LIMIT`          | 最大并发数据库查询数 | `10` |
| `RESILIENCE_LLM_RATE_LIMIT`            | 最大并发 LLM 调用数 | `5`  |
| `RESILIENCE_RATE_LIMIT_TIMEOUT`        | 等待限流槽位的超时（秒） | `30` |
