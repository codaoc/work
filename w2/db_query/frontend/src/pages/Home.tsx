  ...
  return (
    <div style={{ display: "flex", height: "100vh", background: "#F4EFEA" }}>
      ...
      >
        ...
        {/* Export Panel (Always visible when database selected) */}
        {selectedDatabase && (
          <ExportPanel
            queryResult={queryResult}
            databaseName={selectedDatabase}
            onExecuteAndExport={async (format: "csv" | "json") => {
              if (!sql.trim()) {
              ...
              try {
                ...
                setQueryResult(response.data);
                message.success(
                  `查询完成 - ${response.data.rowCount} 行，耗时 ${response.data.executionTimeMs}ms`
                );
